...
<html>
  ...
  <body>
    <div id="intro" class="intro">
      ...
      Max width is 21 mm (2.1cm)<br /><br />
    </div>
    ...
    <script>
      function calculate_dado_stack() {
        let a,
          b,
          c = 0; // number of chippers of width 3.175, 2.38125, 1.5875 respectively
        let d,
          e,
          f,
          g = 0; // number of shims of width 0.127, 0.254, 0.381, 0.508 respectively

        const smallestShimWidth = 0.127;

        let result_message = "";

        let desired_width = document.getElementById("mm_input").value;
        const sawblade_width = 6.35; //width of L&R saw blades)
        const max_width = 21; // ~widest possible dado for table saw due to arbor

        if (desired_width < sawblade_width) {
          result_message =
            "The minimum width is " +
            sawblade_width +
            " Try using a normal saw blade";
        } else {
          if (desired_width > max_width) {
            let passes = Math.ceil(desired_width / max_width);
            desired_width = desired_width / passes;
            result_message +=
              "Due to over-width you will need " +
              passes +
              " passes of width " +
              desired_width;
          }
          ...
          // Magic number calculations done in Excel by binary counting up using the shim widths
          ...
          let max_shims = Math.ceil(remainder / smallestShimWidth);
          let min_shims = Math.floor(remainder / smallestShimWidth);

          let shim_width = min_shims * smallestShimWidth;

          let slightly_over =
            max_shims * smallestShimWidth + chipper_width + sawblade_width;
         ...
         switch(min_shims) {
          ...
          result_message +=
            'Just over width (add another 0.05" shim) = ' +
            slightly_over +
            "mm <br>";
